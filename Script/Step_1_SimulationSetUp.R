########## script for simulation set up
#### Clear the environment
rm(list = ls())

# Define the directory where to run simulations
base_dir <- "D:/simulation_postdoc/sim"
setwd(base_dir)
...
inputDir <- "D:/simulation_postdoc/Landis_inputs"
...
scenariosDesign <- list(area = c("coteNord"),
                        ...
                        mgmt = list("coteNord" = c("0")), # run only BAU under diffrent climate scenarios. in CBM.R we will create our salvage scenarios
                        fire = TRUE,
                        BDA = TRUE,
                        wind = TRUE,
                        harvest = TRUE,
...
#simInfo$simID <- sprintf("%2d", nchar(nrow(simInfo)), 1:nrow(simInfo))
...
foreach(i = 1:nrow(simInfo)) %dopar% {
  ...
  file.copy(paste0(inputDir, "/OutputBiomassCommunity.txt"),
            paste0(simID, "/OutputBiomassCommunity.txt"), overwrite = TRUE)
